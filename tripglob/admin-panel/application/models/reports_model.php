...
class Reports_Model extends CI_Model {
	...
	public function search_hotel_booking($user_type,$user_id=0,$condition){
    	 	$conditionQ = '';
    	 	if($condition != '') {
		     	$conditionQ = " AND ( bg.con_pnr_no LIKE '".$condition."%' ";
		     	$conditionQ .= " OR bg.booking_status LIKE '".$condition."%' ";
		     	$conditionQ .= " OR bg.leadpax LIKE '".$condition."%' ";		     	
		     	$conditionQ .= " OR bg.pnr_no LIKE '".$condition."%' ";		     
		     	$conditionQ .= " OR bg.voucher_date LIKE '".$condition."%' )";	     	
	    	}
		$bd_query = "SELECT * from booking_global as bg left join
		...
		public function search_bus_booking($user_type,$user_id=0,$condition){
    	 	$conditionQ = '';
		...
		left join booking_billing_address as ba on bg.billing_address_id = ba.billing_address_id";
			...
			$bd_query .=" left join user_details as ud on bg.user_id = ud.user_id ";
    		$bd_query .=" where  bg.product_id=8 ";
    ...
    // debug($conditionQ);exit;
		...
		public function search_hotel_booking_new($condition){
			...
			$this->db->join('user_details ud', 'bg.user_id = ud.user_id', 'left');
	...
	public function search_bus_booking_new($condition){
			...
			if (!empty($condition['billing_date'])) {
			$this->db->where('bg.voucher_date >=', date('Y-m-d',strtotime($condition['billing_date'])).' 00:00:00');
			$this->db->where('bg.voucher_date <=', date('Y-m-d',strtotime($condition['billing_date'])).' 23:59:59');						
			}
			if (!empty($condition['created_datetime_from']) && !empty($condition['created_datetime_to'])) {
			$this->db->where('bg.voucher_date >=', date('Y-m-d',strtotime($condition['created_datetime_from'])).' 00:00:00');
			$this->db->where('bg.voucher_date <=', date('Y-m-d',strtotime($condition['created_datetime_to'])).' 23:59:59');			
			}elseif (!empty($condition['created_datetime_from']) && empty($condition['created_datetime_to'])) {
			$this->db->where('bg.voucher_date >=', date('Y-m-d',strtotime($condition['created_datetime_from'])).' 00:00:00');		
			$this->db->where('bg.voucher_date <=', date('Y-m-d').' 23:59:59');			
			}elseif(empty($condition['created_datetime_from']) && !empty($condition['created_datetime_to'])){
			$this->db->where('bg.voucher_date >=', date('Y-m-d',strtotime($condition['created_datetime_to'])).' 00:00:00');
			$this->db->where('bg.voucher_date <=', date('Y-m-d',strtotime($condition['created_datetime_to'])).' 23:59:59');			
			}
			$this->db->select('*');
			$this->db->from('booking_global bg'); 
...
public function insurance_booking_condition($user_type,$user_id=0,$cond){
     	$condition = $this->get_custom_condition_flyonair($cond);
		$bd_query = "SELECT * from booking_global as bg left join
		booking_flight as bf on  bg.referal_id=bf.booking_flight_id  
		left join booking_transaction as bt on bg.booking_transaction_id=bt.booking_transaction_id
    ...
    public function insurance_booking($user_type,$user_id=0){


		// left join booking_billing_address as ba on bg.billing_address_id = ba.billing_address_id

		$bd_query = "SELECT * from booking_global as bg left join
		insurance_book as ib on  bg.referal_id=ib.transaction_id  
		left join booking_transaction as bt on bg.referal_id=bt.app_ref"; 

		if($user_type==B2B_USER || $user_type==STAFF_USER){
			$bd_query .=" left join user_details as ud on bg.user_id = ud.user_id ";
		}
		$bd_query .=" where bg.product_id=10 and bg.user_type_id = ".$user_type;
	...
	public function get_search_det($referal_id)
    {
        $this->db->select('search_id');
    	$this->db->from('insurance_book');
    	$this->db->where('transaction_id', $referal_id);
    	$query = $this->db->get();
    	if ($query->num_rows() == '') {
    	return '';
    	} else {
    ...
    public function get_search_det_global($referal_id)
    {
        $this->db->select('search_id');
	...
	public function get_search_details($search_id)
    {
        $this->db->select('search_data');
