...
class Deposit_Model extends CI_Model {
	...
	public function get_deposit_details($deposit_status,$date="",$deposit_status_admin=''){
		 $this->db->select('user_deposit.*,user_deposit.user_id as user_deposit_id,user_type.*,user_details.*,user_deposit.status as deposit_status,user_accounts.*')
				...
				if($deposit_status!='')
				{
				    // if ($this->session->userdata('admin_id')==1 && $deposit_status=='PENDING') {
				   // 	 $this->db->where('user_deposit.superadmin_status','PENDING'); changed
					if ($this->session->userdata('admin_id')==1) {
						
					 $this->db->where('user_deposit.superadmin_status',$deposit_status);
					}else{
