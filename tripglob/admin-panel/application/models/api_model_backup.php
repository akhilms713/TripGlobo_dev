...
WHERE MONTH(xml_timestamp) = MONTH(CURRENT_DATE - INTERVAL 1 MONTH) AND xml_status = "SUCCESS"')->row();
	...
	public function api_hits(){
		return $this->db->query('SELECT a.api_details_id,a.api_name,COUNT(xl.api_details_id) as TOTAL_API_HITS FROM xml_logs xl
									LEFT JOIN `api` a ON(a.api_details_id = xl.api_details_id) GROUP BY xl.api_details_id')->result();
	}
