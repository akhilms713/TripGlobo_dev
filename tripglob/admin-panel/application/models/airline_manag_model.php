<?php
class Airline_Manag_Model extends CI_Model {
    ...
    function get_airline_list($limit = '',$start='',$search=""){
		$this->db->select('*');
		$this->db->from('airline_list');
		 if($search != ''){
              $this->db->like('airline_code', $search);
              $this->db->or_like('airline_name', $search);
            }
	...
	function get_airline_list_row($search=""){
		$this->db->select('*');
		$this->db->from('airline_list');
		 if($search != ''){
              $this->db->like('airline_code', $search);
	...
	function get_airline_list_single($airline_details_id = ''){
		$this->db->select('*');
		$this->db->from('airline_list');
		if($airline_details_id !='')
			$this->db->where('airline_list_id', $airline_details_id);		
    ...
    function add_airline_details($input,$airline_logo_name){
		...
		$insert_data = array(
							...
							'airline_image' 		=> $airline_logo_name,							
							'status' 				=> $input['status'],
						);			
	...
	function update_airline_status1($id,$status)
    {
        $data = array(
            'status' => $status       
            );
        
            $where = "airline_list_id ='$id'";

        if ($this->db->update('airline_list', $data, $where)) {
