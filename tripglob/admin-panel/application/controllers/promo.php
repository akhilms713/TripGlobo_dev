...
// 			$exp_date = $this->input->post('exp_date');
	...
	public function send_promo_to_user_do($id)
	{
		...
		{	
				...
				$user_list = $this->promo_model->get_user_list_details_new(implode(",",$_POST['user_type'])); 
				for($i=0;$i<count($user_list);$i++)
				{
					$email_user[]  = $user_list[$i]->user_email_id;
				}
		...
		$message = '';
		if ($promodata->user_id == 0 || $promodata->user_type == 2) {
		$email=array_merge($str_arr,$email_user);				
		}else{
			$userdata=$this->db->select('user_email_id')->get_where('user_login_details',array('user_id'=>$promodata->user_id))->row_array();
			$useremail[]=$userdata['user_email_id'];
		$email=array_merge($str_arr,$useremail);				
		}
		// debug($email);exit;
		...
		if($promo_arr){
			...
			for($j=0;$j<count($email);$j++){
				// echo $str_arr[$j];
				if($email[$j]!=''){					
				$this->email_model->sendpromomail($email[$j],$_POST['subject'],$message);
				}
				$res='true';
			}
