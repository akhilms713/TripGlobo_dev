        ...
        try { 
             ...
             $(".fromflight").autocomplete({
               ...
               select: function(event,ui){
                  ...
                  // console.log(ui.item.id,"sangaram");
                  $("#to").focus();
               }
             })
...
// transfer
       try { 
             ...
             $('#country').on('change', function(){
              let countrys = $(this).val();  
             $(".fromtransfer").autocomplete({
               
               // source: WEB_URL+"general/transfer_search/",
                   source: WEB_URL + "general/transfer_search/countrys?country=" + encodeURIComponent(countrys),
               minLength: 2,//search after two characters     
               select: function(event,ui){
                  ...
                  if(ui.item.hasOwnProperty("id")==true){
                    console.log(ui.item);
                     $("#from_loc_id").val(ui.item.id);
                     $("#from_city_id").val(ui.item.city_id); 
                     $("#from_HotelId").val(ui.item.from_HotelId);
                     $("#from_station_ids").val(ui.item.from_station_id);
     ...
     $(".droptransfer").autocomplete({
       ...
       select: function(event,ui){
        console.log(ui.item);
          if(ui.item.hasOwnProperty("id")==true){
            $("#to_loc_id").val(ui.item.id);
            $("#to_station_id").val(ui.item.city_id); 
            $("#to_HotelId").val(ui.item.to_HotelId);
            $("#to_station_code").val(ui.item.from_station_id);
          }else{
            $("#to_loc_id").val(0);
          }
           // $("#depature").focus();
      ...
      if(mymvaluexx==""){
        ...
        if (cnt = 2){
          ...
          if(selectedDateValue == ''){
           ...
           var WEB_URL2='https://tripglobo.com/index.php/';
 ...
 var WEB_URL='https://tripglobo.com/index.php/';
       ...
       $(".departflightm").autocomplete({
         ...
         select: function(event,ui){
            ...
            var dat=$("#to_loc_id_1").val(ui.item.id);
            var id_v = $(this).attr('data-id');
            $("#depature_m_" + id_v).focus();
            // console.log("this is for multicity 1");
          //  var refid = $(this).attr('data-refid');
           //$('#depature_multi'+refid).focus();
