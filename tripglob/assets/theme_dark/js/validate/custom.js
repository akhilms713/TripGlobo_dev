	...
	submitHandler: function() { 
		...
		$.ajax({
			...
			beforeSend: function() { 
                ...
                var loadewr = 'https://tripglobo.com/assets/theme_dark/images/preloader.gif';
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.status == 1) {
					...
					$('#total_amount').val(js_finalAmt);
					$('#pcode').val(data.code);

					$('#pcode_d').val(data.discount);
					$('#pcode_f').val(data.finalAmt);
					$('#discount_row').removeClass('hide');					
					$('.cancel_booking_details').show();					
selectElement('currency_list', 'INR');
// $('#curr_symbol').val('र');
$('#curr_symbol').html('र');
				}else if(data.status == 0) {
					$('.savemessage').html(data.discMsg).show();
