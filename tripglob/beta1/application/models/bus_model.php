...
Class Bus_Model extends CI_Model
{
	...
	function get_bus_station_list($search_chars)
	{
		...
		$query = 'Select * from tbo_bus_city_list where CityName like '.$search_chars.'
		ORDER BY CityName ASC LIMIT 0, 20';
	
		return $this->db->query($query)->result_array();
	}
	function get_bus_station_data($CityName)
	{
		// print_r($id);exit;
		$query = "Select * from tbo_bus_city_list where CityName ='".$CityName."'";
	
		return $this->db->query($query)->row();
	}

	function get_bus_station_data_tmx($id)
	{
		// print_r($id);exit;
		$query = "Select * from bus_stations_new where name ='".$id."'";
	...
	function active_booking_source()
	{
		...
		/*new added*/
		// $query = 'select BS.source_id, BS.origin , MCL.status from meta_course_list AS MCL, booking_source AS BS, activity_source_map AS ASM WHERE
		// MCL.origin=ASM.meta_course_list_fk and ASM.booking_source_fk=BS.origin and MCL.course_id='.$this->db->escape(META_BUS_COURSE).'
		// and BS.booking_engine_status='.ACTIVE.' AND MCL.status='.ACTIVE.' AND ASM.status="active"';
		// return $this->db->query($query)->result_array();
	}
	...
	function get_search_data($search_id)
	{
		...
		if (empty($this->master_search_data)) {
			$search_data = $this->custom_db->single_table_records('search_history', '*', array('search_type' => "BUS", 'origin' => $search_id));
		...
		// return $this->master_search_data;
				return array('status' => $status, 'data' => $this->master_search_data);
	}
	...
	/*new added */
	function insert_tbo_city_list($cities)
	{
		
		foreach ($cities['BusCities'] as $value) {
			// debug($value);exit;
			$CityId = $value['CityId'];
			$CityName = $value['CityName'];
			$query = "INSERT INTO tbo_bus_city_list(CityId,CityName) VALUES ('".$CityId."','".$CityName."')";
			$this->db->query($query);

		}
	}
	
	function insert_tbo_city_list_new($cities)
	{
		// debug($cities['BusCities']);exit;
          $this->db->insert_batch('tbo_bus_city_list',$cities['BusCities']);
		// foreach ($cities['BusCities'] as $value) {
		//  // debug($value);exit;
		// 	$CityId = $value['CityId'];
		// 	$CityName = $value['CityName'];
...
/*
	...
	function ins_book_data($search_id ='', $book_data='', $shopping_global_id='') {
   		/*debug($search_id);
   		debug($cart_data);exit();*/
		$cart_data = array(				
			'search_id' => $search_id,				
			'global_id' => $shopping_global_id,				
			'book_data' => $book_data				
		);
		$this->db->insert('ins_bus_book_data',$cart_data);
		
	}
	function add_cart_details_bus($cart_data) {
	   ...
	   if($this->session->userdata('user_id')){
		...
		$this->db->insert('cart_bus',$cart_data);
        $cart_bus_id = $this->db->insert_id();
		
        $cart_global = array(
				'parent_cart_id' => 0,
				'referal_id' => $cart_bus_id,
				'product_id' => '8',
				'user_type' => $user_type,
				'user_id' => $user_id,
				'session_id' => $cart_data['session_id'],
				'site_currency' => BASE_CURRENCY,
				'total_cost' => $cart_data['amount'],
        ...
        $this->db->query("UPDATE cart_bus SET parent_cart_id='$cart_global_id' WHERE origin='$cart_bus_id'"); 
        
        $cdata['cart_bus_id']      = $cart_bus_id;
    ...
    public function bus_xml_logger($operation_name, $app_reference, $global_id, $cart_id, $request, $response)
	{
		...
		$this->custom_db->insert_record('bus_xml_logger', $data);
	}
	
	function get_markup_B2C($markup_type, $user_type,$product_details_id,$api_details_id,$total_amount) {
		$admin_markup=0;
		if ($user_type==0) {
			$user_type=2;
		}
		// debug($user_type);exit();
       $markup = $this->db->get_where('markup_details',array('markup_type'=>$markup_type,'user_type_id'=>$user_type,'product_details_id'=>$product_details_id,'api_details_id'=>$api_details_id,'status'=>'ACTIVE'))->row_array();
       // debug($this->db->last_query());exit();
           if (!empty($markup)) {           	
				if (!empty($markup['markup_value'])) {
					$percentage = $markup['markup_value'];
					$admin_markup = $this->PercentageToAmount($total_amount, $percentage);   
					} else {
					$admin_markup = $markup['markup_fixed'];
				}
           }
           return $admin_markup;
    }
