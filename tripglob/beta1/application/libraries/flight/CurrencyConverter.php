...
class CurrencyConverter{
   ...
   public function get_currentCurrency()
   {
	...
	$default_currency = $CI->session->userdata('default_currency');
	$default_currency_array = array();
		...
		if(!isset($default_currency) || empty($default_currency)){
			...
			if(!isset($default_currency) || empty($default_currency)){
				$default_currency_array=explode('|', 'USD|$');
			} else {
				$default_currency_array=explode('|', $config_currency);
			}
		} else {
			$default_currency=json_decode($default_currency);
			$default_currency_array[]=trim($default_currency->code);
			$default_currency_array[]=trim($default_currency->icon);
		} 
   ...
   public function convert($from_currency,$to_currency,$amount)
   {
    ...
    private function checkIfExistTable(){
        ...
        if ($CI->db->table_exists('currency_details') ){
            return(true);
        }
        else{
            $CI->load->dbforge();
            ...
            $CI->dbforge->create_table('currency_details',TRUE);
        } 
    }
}
