 ...
 if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    function set_credentials() {
        ...
        $api_usage = $api_data[0]["api_credential_type"];  
            ...
            $data['Url_auth']                 = $api_data[0]["api_url"];
...

    {
       ...
       $request['data']['service_url'] = $data['Url_auth'];
      ...
      // debug($header_info);die();
       // debug($data);
        $CI=get_instance();
        $token=$CI->db->get_where('authenticate_token',array('type'=>'BUS'))->row_array();
        if (date('Y-m-d',strtotime($token['create_at']))==date('Y-m-d') && json_decode($token['data'],1)['Status']== true) {
      // debug($data); 
            // debug($token);exit;
            $response_data=json_decode($token['data'],1);    
        }else{ 
            $response_data = get_aut_json_response($request['data']['service_url'], $header_info);      
            $CI->db->where('type','BUS');
             $token_data['data']=json_encode($response_data);
    ...
    function bus_search_request($bus_station_from_id, $bus_station_to_id, $bus_date, $number_of_seats = 1, $SearchId = 123456,$authresponse) {
        ...
        // $request_paramas['EndUserIp'] = "127.0.0.1";
        ...
        $data = set_credentials();
        // debug($data);exit;
        $response['data']['service_url'] = $data['Url'].'/Search/';
        // $response['data']['service_url'] = 'http://api.tektravels.com/BookingEngineService_Bus/BusService.svc/rest/Search/';
        return $response;
    }
...
//live
    ...
    function GetBoarding_request($EndUserIp, $resultIndex, $traceId, $tokenId) {
        ...
        $data = set_credentials();
        $url = $data['Url'].'/GetBoardingPointDetails/';
    ...
    function HoldSeatsForSchedule_request($params) {
        ...
        unset($seatarray['Price']['TotalFare']);

        unset($seatarray['Price']['agent_markup_price']);
        unset($seatarray['Price']['admin_markup']);
    ...
    // debug($post_params);exit;     
        ...
        $request_paramas['BoardingPointId'] = $post_params['token']['PickUpID'];
        $request_paramas['DropingPointId'] = $post_params['token']['DropID'];
                    ...
                    foreach ($SeatDetails as $seatkey => $SeatDetails_v) {
                        ...
                        foreach($SeatDetails_v as $bus_key => $bus_result){
                           // $bus_result['Price']['admin_markup']=0;    
                                ...
                                $getBusMarkup = $CI->bus_model->get_markup($bus_result['Price']['PublishedPriceRoundedOff']);
                                 $bus_result['Price']['admin_markup']=$getBusMarkup['admin_markup'];
                                 $bus_result['Price']['agent_markup_price']=$getBusMarkup['agent_markup'];
                                 $bus_result['Price']['TotalFare']=array_sum($getBusMarkup) + $bus_result['Price']['PublishedPriceRoundedOff'];
 ...
 // debug($res);exit;
    ...
    function block_seats($search_id, $block_params='') {
        ...
        if (valid_array($block_params)) {
            ...
            $header_info = get_header();
            if ($request['status']) {
                $response_data=json_decode(file_get_contents(PROJECT_PATH.'/logs/Bus/search/Block_Res20240702_153319_7120.json'),1);
                // $response_data = get_json_response($request['data']['service_url'], $request['data']['request'], $header_info);                
    ...
    function process_booking($blockresponse, $post_params) {    
        ...
        $resposne['msg'] = 'Remote IO Error';
            ...
            //get data
            if ($request['status']) {
                 $response_data=json_decode(file_get_contents(PROJECT_PATH.'/logs/Bus/search/Book_Res20240702_153321_3367.json'),1);
                // $response_data = get_json_response($request['data']['service_url'], $request['data']['request'], $header_info); 
                // debug($response_data);exit;              
     ...
     */
     ...
     function format_search_response($search_result,  $search_id, $module,$module_type='2') {
        ...
        foreach($search_result as $bus_key => $bus_result){
                ...
                $getBusMarkup = $CI->bus_model->get_markup($bus_result['BusPrice']['PublishedPriceRoundedOff']);
            // debug($bus_result['BusPrice']['PublishedPriceRoundedOff']);exit;
                $bus_result['BusPrice']['admin_markup']=$getBusMarkup['admin_markup'];
                $bus_result['BusPrice']['agent_markup_price']=$getBusMarkup['agent_markup'];
...
function get_bus_city_list_new()
{
  ...
  CURLOPT_POSTFIELDS =>'{
    "TokenId": "f085ecdb-9904-4e6b-89d1-2ddb811ed78b",
    "IpAddress": "192.168.10.100",
    "ClientId": "ApiIntegrationNew",
}',
  CURLOPT_HTTPHEADER => array(
    'Content-Type: application/json',
    'f085ecdb-9904-4e6b-89d1-2ddb811ed78b'
  ),
));
