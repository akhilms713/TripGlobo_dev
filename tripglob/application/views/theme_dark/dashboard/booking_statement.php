...
<div class="main-chart">
 <h3 class="lineth">Booking Management</h3>
...
<?php } ?>
    ...
    <div class="errstatus" style="display: none;"></div>
       ...
       <!--  <span class="profile_head">Booking Management</span>
     -->
          <div id="advance_search_form_container" class="serch_area_fltr">
   <form action="<?=base_url()?>dashboard/booking_statement" method="get" autocomplete="off" class="form-horizontal" role="form">
...
<div class="searc_fliter_all">
   ...
   <?php 
    ...
    ?>
     <a class="<?=(($today_search == @$_GET['today_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement?today_booking_data=<?=date('Y-m-d')?>">Today Search</a>
       <a class="<?=(($last_today_search == @$_GET['last_day_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement?last_day_booking_data=<?=date('Y-m-d', strtotime('-1 day'))?>">Last Day Search</a>
       <a class="<?=(($last_3_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement?prev_booking_data=<?=date('Y-m-d', strtotime('-3 day'))?>">Last 3 Day Search</a>
       <a class="<?=(($last_7_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement?prev_booking_data=<?=date('Y-m-d', strtotime('-7 day'))?>">Last 7 Day Search</a>
       <a class="<?=(($last_15_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement?prev_booking_data=<?=date('Y-m-d', strtotime('-15 day'))?>">Last 15 Day Search</a>
       <a class="<?=(($last_1m_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement?prev_booking_data=<?=date('Y-m-d', strtotime('-1 month'))?>">Last 1 Month Search</a>
       <a class="<?=(($last_3m_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement?prev_booking_data=<?=date('Y-m-d', strtotime('-3 month'))?>">Last 3 Month Search</a>
   </div>
</div>
<div class="clearfix"></div>
<div class="clearfix"></div>
                ...
                <table id="depostDatatable" class="data-table-column-filter table table-bordered table-striped" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo date("d-m-Y", strtotime($k->voucher_date)); ?></td>
                                
                              
                                <td><?php echo $k->booking_status; ?></td>
                               
                                 <?php
                                  if($k->product_name == 'FLIGHT'){
                                       $booking = $this->account_model->getBookingbyPnr($k->parent_pnr_no,$k->product_name)->row();
                                       $booking_transaction = $this->account_model->booking_transaction_data($booking->booking_transaction_id);

                                       
                                       ?>
                                       <td><?php echo $booking_transaction['total_amount']-($booking_transaction['agent_markup']+$booking_transaction['discount']); ?></td>
                                <td><?php echo $booking_transaction['total_amount']; ?></td>
                                <td><?php echo $booking_transaction['agent_markup']+$booking_transaction['discount']; ?></td>
                                  <?php }elseif ($k->product_name =='HOTEL') {
                                       $hoteldata=$this->account_model->get_hoteldatainfo_new($k->parent_pnr_no);?>
                                   <td><?php echo $hoteldata['total_cost']-($hoteldata['agent_markup']+$hoteldata['discount']); ?></td>
                                <td><?php echo $hoteldata['total_cost']; ?></td>
                                <td><?php echo $hoteldata['agent_markup']+$hoteldata['discount']; ?></td>
                                   <?php }else{
                                      $cart_data=$this->account_model->cart_data_bus($k->cart_id);
                                    ?>

                                <td><?php echo $cart_data['amount']-($cart_data['agent_markup']+$cart_data['discount']); ?></td>
                                <td><?php echo $cart_data['amount']; ?></td>
                                <td><?php echo $cart_data['agent_markup']+$cart_data['discount']; ?></td>
                                  <?php }
                                ...
                                <td>                               
                                <?php 
                                    $voucher_url =  WEB_URL.'voucher/'.base64_encode(base64_encode($k->booking_global_id)).'/'. base64_encode(base64_encode($k->product_id));
                                    // $invoice_url = WEB_URL.'invoice/'.base64_encode(base64_encode($booking->parent_pnr_no));
                                   
                                ?>
                                <a href="<?php echo $voucher_url ;?>" target="_blank" title="View voucher" data-placement="top" class="btn btn-success btn-xs has-tooltip" data-original-title="View Voucher">
                                    <i class="icon-ticket"></i>View Details
                                </a>
                                
                                  <!--  <a href="<?php echo $invoice_url;?>" target="_blank" title="View Invoice" data-placement="top" class="btn btn-primary btn-xs has-tooltip" data-original-title="View Invoice">
