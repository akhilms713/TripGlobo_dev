...
// echo "<pre>"; print_r($CUR_Layout); exit('shobik');

// debug($CUR_Layout);exit;

$sl_max_rows = $CUR_Layout['SeatLayout']['NoOfRows'];

$sl_max_cols = $CUR_Layout['SeatLayout']['NoOfColumns'];
...
// 	debug($sl_bus_seats);exit;
	...
	$sl_bus_deck = $CUR_Layout['SeatLayout'];

	$counttoken = count($sl_bus_deck['SeatDetails']);

	foreach ($sl_bus_deck['SeatDetails'] as $deck_k => $deck_v) {

		



		foreach ($deck_v as $key => $value) {
		...
		if ($value['IsUpper'] == false) {
			...
			$seat_layout .= '<tr><td rowspan="">'.$deck_name.'</td></tr>';
		...
		// for ($col = $deck_v['max_col']; $col >= $deck_v['min_col']; $col--) {
			...
			// for($row = $deck_v['min_row']; $row <= $deck_v['max_row']; $row++) {
				...
				if($value['IsUpper'] == true){
					...
					if ($value['IsUpper'] == true && !empty($value['SeatIndex'])) {

					$seat_number = $value['SeatIndex'];

					$seat_deck = $value;

					$seat_no_summary[$seat_number] = $value;

					$first_fare = $value['SeatFare'];

					$Markup = $fare_details['default_value'] - $first_fare;

					//update Fare

					$fare_details = $currency_obj->get_currency($value['SeatFare']);
					...
					$value['SeatFare'] = $seat_no_summary[$seat_number]['Fare'] = $fare_details['default_value']+$gst_value;
					...
					if ($value['SeatStatus'] == '1') {
						...
						else if($value['IsMalesSeat'] == false && $value['IsLadiesSeat'] == false){
						...
						if($value['SeatStatus'] == false){
							...
							$sl_title = array('Seat : '.$seat_number, ' Fare : '.$value['SeatFare']);
						...
						}else if ($value['SeatType'] == 4) {

							//vertical sleeper

							$seat_type = 'UpperBearth';
						...
						}else if ($value['SeatType'] == 5) {

							//vertical sleeper

							$seat_type = 'LowerBearth';
				...
				else if($value['IsUpper'] == false){

					// debug($value);

					if ($value['IsUpper'] == false && !empty($value['SeatIndex'])) {
				    ...
				    // debug( $gst_value);
				  	...
				  	// debug($fare_details['default_value']+$gst_value);

					$value['SeatFare'] = $seat_no_summary[$seat_number]['Fare'] = $fare_details['default_value']+$gst_value;

					// debug($value['SeatFare']); exit;
