...
<?php } ?>
   ...
   <form action="<?=base_url()?>dashboard/booking_statement1" method="get" autocomplete="off" class="form-horizontal" role="form">
...
<div class="searc_fliter_all">
   ...
   <?php 
    ...
    ?>
     <a class="<?=(($today_search == @$_GET['today_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement1?today_booking_data=<?=date('Y-m-d')?>">Today Search</a>
       <a class="<?=(($last_today_search == @$_GET['last_day_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement1?last_day_booking_data=<?=date('Y-m-d', strtotime('-1 day'))?>">Last Day Search</a>
       <a class="<?=(($last_3_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement1?prev_booking_data=<?=date('Y-m-d', strtotime('-3 day'))?>">Last 3 Day Search</a>
       <a class="<?=(($last_7_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement1?prev_booking_data=<?=date('Y-m-d', strtotime('-7 day'))?>">Last 7 Day Search</a>
       <a class="<?=(($last_15_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement1?prev_booking_data=<?=date('Y-m-d', strtotime('-15 day'))?>">Last 15 Day Search</a>
       <a class="<?=(($last_1m_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement1?prev_booking_data=<?=date('Y-m-d', strtotime('-1 month'))?>">Last 1 Month Search</a>
       <a class="<?=(($last_3m_search == @$_GET['prev_booking_data']) ? 'active' : '')?>" href="<?=base_url()?>dashboard/booking_statement1?prev_booking_data=<?=date('Y-m-d', strtotime('-3 month'))?>">Last 3 Month Search</a>
   </div>
</div>
<div class="clearfix"></div>
<div class="clearfix"></div>
                ...
                <table id="depostDatatable" class="data-table-column-filter table table-bordered table-striped" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                ?>
                                <td><?php echo $booking->total_amount-$booking->agent_markup; ?></td>
                                <td><?php echo $booking->total_amount; ?></td>
                                <td><?php echo $booking->agent_markup; ?></td>
                                
                                <td>                               
                                <?php 
                                    $voucher_url =  WEB_URL.'voucher/'.base64_encode(base64_encode($booking->parent_pnr_no)).'/'.base64_encode(base64_encode($k->product_id));
                                    $invoice_url = WEB_URL.'invoice/'.base64_encode(base64_encode($booking->parent_pnr_no)).'/'.base64_encode(base64_encode($k->product_id));
