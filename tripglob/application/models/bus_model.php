...
/*
	...
	function ins_book_data($search_id ='', $book_data='', $shopping_global_id='') {
   		...
   		$update_record=$this->db->get_where('ins_bus_book_data',array('search_id'=>$search_id))->num_rows();
   		if ($update_record==0) {
		$cart_data = array(				
			'search_id' => $search_id,				
		...
		$this->db->update('ins_bus_book_data',$cart_data);
   		}
	...
	function get_markup($TotalPrice) {
...
// debug($generic_markup_B2C);exit;
            ...
            $totel_markup['admin_markup']=$admin_markup;
            $totel_markup['agent_markup']=$agent_markup_price;
            // debug($totel_markup);
    ...
    function get_markup_B2B($markup_type, $B2B) {
        ...
        $this->db->where('product_details_id', 4);
